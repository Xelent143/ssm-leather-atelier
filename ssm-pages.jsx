...
function ShippingReturns({ go, mode = 'shipping' }) {
  ...
  const returnHighlights = [
    ['30 DAYS', 'Standard-piece returns', 'Request a return or exchange within 30 days of signed delivery.'],
    ['2 WORKING DAYS', 'Incorrect or faulty items', 'Tell us promptly and include clear photographs so we can review the issue.'],
    ['7 BUSINESS DAYS', 'Approved refunds', 'After inspection, an approved refund is initiated to the original payment method.'],
  ];
  const returnSections = [
    {
      label: 'STANDARD RETURNS & EXCHANGES',
      title: 'A clear 30-day window.',
      paragraphs: [
        'Eligible standard-size, non-personalized pieces may be returned or exchanged within 30 days of the date the parcel is signed for. Submit a return request before sending anything back; unauthorized parcels may be refused.',
        'Once a return reaches MOTOGRIP GEAR, we normally inspect it within 72 hours. If approved, we initiate the refund within 7 business days to the original payment method. Your bank or card provider may take up to 10 working days to show the credit.',
      ],
    },
    {
      label: 'ELIGIBILITY & CONDITION',
      title: 'Return it as received.',
      items: [
        ['Original condition', 'The piece must be unworn, unused, unwashed, unaltered and undamaged, with all original tags, accessories and packaging included. Please enclose the order details or packing slip.'],
        ['Personalized and custom pieces', 'Made-to-measure, bespoke, monogrammed, engraved or otherwise personalized products are created specifically for you and are not eligible for a standard return or refund.'],
        ['Final-sale items', 'Gift cards, factory seconds and products clearly marked final sale cannot be returned or exchanged. Promotional items follow the terms displayed when the order is placed.'],
        ['Incorrect or faulty item', 'Contact us within 2 working days of delivery with the order number and photographs. We will review the concern and confirm the appropriate remedy.'],
      ],
    },
    {
      ...
      paragraphs: [
        'The customer is responsible for return postage, insurance and any carrier charges unless MOTOGRIP GEAR confirms that the item supplied was incorrect or faulty. Original delivery charges and import duties are not refundable except where required by law.',
        'Use a tracked service and retain the receipt until the return is complete. MOTOGRIP GEAR cannot accept responsibility for a return parcel until it is delivered to the authorized return address.',
      ],
    },
    {
      label: 'CUSTOM & MADE-TO-MEASURE',
      title: 'Built for you, supported by us.',
      paragraphs: [
        'Before production, we may confirm measurements, design details and reference images with you. After those details are approved and cutting begins, the order cannot be treated as a standard return.',
        'If a custom piece needs a fit correction, the first fit alteration is complimentary when requested within 60 days of receipt. Alteration support is assessed against the measurements and specifications approved for the order. Return shipping or later alterations may be chargeable.',
        'Custom and made-to-measure orders are not refundable. This does not limit any remedy that the law requires for a product that is faulty or not made to the agreed specification.',
      ],
    },
    {
      label: 'CANCELLATIONS',
      title: 'Tell us before work begins.',
      paragraphs: [
        'A cancellation request must be submitted within 2 days of placing the order. We cannot guarantee cancellation after production has started, an order has entered customs clearance or a parcel has been dispatched.',
        'Rush or time-sensitive orders may enter production immediately and cannot be cancelled once work begins. If we can stop an eligible order, we will confirm the cancellation and any applicable refund in writing.',
      ],
    },
    {
      label: 'ORDER ACCEPTANCE & PRICING',
      title: 'Accurate, fair and transparent.',
      paragraphs: [
        'An order confirmation acknowledges your request; final acceptance occurs when MOTOGRIP GEAR confirms or dispatches the order. We may decline an order for availability, payment, suspected fraud, shipping restrictions, legal requirements or an obvious pricing or description error.',
        'If a genuine pricing error is found before acceptance, we will contact you with the correct price and let you continue or cancel. If payment has already been captured for an order we cannot accept, the applicable amount will be returned to the original payment method.',
        'Nothing in this policy removes statutory consumer rights that apply in the customer’s country or state.',
      ],
    },
  ];
  return (
    <div className="page-fade">
      <PageHero
        ...
        italic={isReturns ? 'made clear.' : 'shipping available.'}
        dek={isReturns ? 'Straightforward terms for standard pieces, custom work, exchanges, alterations, refunds and cancellations.' : undefined}
      />
      ...
      <section style={{ padding: '0 48px 96px', maxWidth: 1080, margin: '0 auto' }}>
        ...
        {isReturns && <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 240px), 1fr))', borderTop: '1px solid var(--line)', marginBottom: 18 }}>
            {returnHighlights.map(([number, title, body]) => (
              <div key={title} style={{ padding: '28px 24px', borderBottom: '1px solid var(--line)' }}>
                <div className="mono" style={{ color: 'var(--accent-2)', fontSize: 10, marginBottom: 14 }}>{number}</div>
                <div style={{ fontFamily: 'var(--display)', fontSize: 25, marginBottom: 10 }}>{title}</div>
                <div style={{ color: 'var(--fg-3)', fontSize: 14, lineHeight: 1.72 }}>{body}</div>
              </div>
            ))}
          </div>

          {returnSections.map((section) => (
            <div key={section.label} style={{ padding: '54px 0', borderBottom: '1px solid var(--line)' }}>
              <div className="mono" style={{ color: 'var(--accent-2)', fontSize: 10, marginBottom: 16 }}>{section.label}</div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 320px), 1fr))', gap: 48 }}>
                <div style={{ fontFamily: 'var(--display)', fontSize: 'clamp(28px, 3.6vw, 44px)', lineHeight: 1.08 }}>{section.title}</div>
                <div>
                  {section.paragraphs && section.paragraphs.map((paragraph) => (
                    <p key={paragraph} style={{ color: 'var(--fg-3)', fontSize: 14, lineHeight: 1.82, margin: '0 0 16px' }}>{paragraph}</p>
                  ))}
                  {section.items && section.items.map(([title, body]) => (
                    <div key={title} style={{ paddingBottom: 20, marginBottom: 20, borderBottom: '1px solid var(--line)' }}>
                      <div style={{ fontFamily: 'var(--display)', fontSize: 22, marginBottom: 8 }}>{title}</div>
                      <div style={{ color: 'var(--fg-3)', fontSize: 14, lineHeight: 1.82 }}>{body}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, paddingTop: 36 }}>
            <button className="btn" onClick={() => go('file-return')}>File a Return</button>
            <button className="btn btn-ghost" onClick={() => go('contact')}>Contact MOTOGRIP GEAR</button>
          </div>
        </>}
      </section>
    </div>
...
function LegalPage({ kind, go }) {
  ...
  const sections = isPrivacy ? [
    ...
    {
      t: 'Website information and corrections',
      p: [
        'We work to keep website information complete, current, and accurate, but occasional typographical errors, omissions, or outdated details may occur in product descriptions, prices, promotions, shipping charges, delivery estimates, or availability.',
        'We may correct material errors, update information, or cancel an affected order where reasonably necessary, including after an order has been submitted. If a correction materially affects an accepted order, we will contact you and provide any remedy required by applicable law.',
      ],
    },
    {
    ...
    },
    {
      ...
      p: [
        'The website may link to, integrate with, or provide access to independent services and optional tools, including payment processors, carriers, mapping, communication, or social platforms. Their services are governed by their own terms and privacy notices.',
        'We do not control third-party services and are not responsible for their content, availability, or conduct, except to the extent responsibility cannot lawfully be excluded. Review the relevant provider’s terms before using its service or completing a transaction with it.',
      ],
    },
    {
      t: 'Comments, feedback, and submissions',
      p: [
        'If you send us a review, photograph, design idea, suggestion, or other submission, you confirm that you have the right to provide it and that it is accurate, lawful, and does not violate another person’s intellectual-property, privacy, or other rights.',
        'You retain ownership of your original content. Where you ask us to publish or use it, you grant MOTOGRIP GEAR a non-exclusive, worldwide, royalty-free licence to store, reproduce, adapt, and display that content for the agreed purpose. We may remove unlawful, misleading, abusive, or infringing content. Private made-to-measure photographs and measurements are handled under our Privacy Policy and are not licensed for public marketing without separate permission.',
      ],
    },
    {
      t: 'Personal information',
      p: [
        'Our collection and use of personal information is governed by the MOTOGRIP GEAR Privacy Policy. By providing personal information, you confirm that it is accurate and that you are authorised to provide it. Do not submit another person’s measurements, photographs, or contact details without their permission or another lawful basis.',
      ],
    },
    {
    ...
    },
    {
      t: 'Indemnity',
      p: [
        'To the extent permitted by law, you are responsible for reasonable losses, claims, or costs arising directly from your unlawful use of the website, your infringement of another person’s rights, or your material breach of these Terms. This section does not require a consumer to compensate us for losses caused by MOTOGRIP GEAR or remove any protection that cannot lawfully be waived.',
      ],
    },
    {
      t: 'Suspension and termination',
      p: [
        'You may stop using the website at any time. We may restrict or terminate access where reasonably necessary to protect the website, customers, or MOTOGRIP GEAR; respond to unlawful or fraudulent activity; or address a material breach of these Terms. Rights and obligations relating to accepted orders, payments, intellectual property, liability, and disputes survive where their nature requires it.',
      ],
    },
    {
    ...
    {
      t: 'Entire agreement, waiver, and severability',
      p: [
        'These Terms and the policies referenced on the website form the agreement governing your use of the service, alongside any specific written terms we agree for an order. If we do not immediately enforce a provision, that does not waive our right to enforce it later.',
        'If any provision is held unlawful or unenforceable, it will be limited or removed only to the minimum extent necessary, and the remaining provisions will continue to apply.',
      ],
    },
    {
      t: 'Changes to these Terms',
      p: ['We may update these Terms by posting a revised version with a new effective date. Changes apply prospectively unless law requires otherwise. Your continued use of the website after an update means the revised Terms apply to that later use; an accepted order remains subject to the terms that applied when it was accepted unless we agree otherwise or law requires a change.'],
    },
  ];
