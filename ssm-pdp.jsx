        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 8 }}>
          {images.map((image, index) => (
            <button key={image.src} onClick={() => setImgIdx(index)} aria-label={image.label}
              ...
              style={{ width: 64, aspectRatio: '3/4', cursor: 'pointer', outline: imgIdx === index ? '1px solid var(--fg)' : 'none', outlineOffset: 2, opacity: imgIdx === index ? 1 : 0.65, background: p.cat === 'Bags' ? '#fff' : 'transparent', border: 0, padding: 0 }}>
              <img src={image.src} alt="" style={{ width: '100%', height: '100%', objectFit: p.cat === 'Bags' ? 'contain' : 'cover', objectPosition: 'center' }} />
            </button>
          ))}
        </div>
        <div style={{ paddingLeft: 24 }}>
          {images.length > 0 && <div className="ph grain" data-label=""
            role="img" aria-label={images[imgIdx]?.label} style={{ aspectRatio: '4/5', background: p.cat === 'Bags' ? '#fff' : 'transparent' }}>
            <img src={images[imgIdx]?.src} alt="" style={{ width: '100%', height: '100%', objectFit: p.cat === 'Bags' ? 'contain' : 'cover', objectPosition: 'center' }} />
          </div>}
        </div>
      ...
      <div className="pdp-commerce-layout" style={{
        ...
        <div className="pdp-main-media" style={{ paddingLeft: 0 }}>
          <div className="ph grain pdp-main-image" data-img="1" data-label=""
            style={{ height: 'min(720px, calc(100vh - 330px))', minHeight: 560, position: 'relative', '--img': `url(${images[imgIdx].src})` }}>
