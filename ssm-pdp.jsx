...
function FactualPDP({ product: p, go, addToCart }) {
  ...
  const selectedPrice = Number(selectedVariant?.price ?? p.price ?? 0);
  ...
  return (
    <div className="page-fade factual-pdp">
      ...
      <div className="factual-pdp-grid" style={{ display: 'grid', gridTemplateColumns: '80px minmax(0, 1fr) 480px', padding: '0 48px 80px' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 8 }}>
          {images.map((image, index) => (
            <button key={image.src} onClick={() => setImgIdx(index)} aria-label={image.label}
              ...
              style={{ width: 64, aspectRatio: '3/4', cursor: 'pointer', '--img': `url(${image.src})`, outline: imgIdx === index ? '1px solid var(--fg)' : 'none', outlineOffset: 2, opacity: imgIdx === index ? 1 : 0.65, background: 'transparent', border: 0 }} />
          ))}
        </div>
        <div style={{ paddingLeft: 24 }}>
          {images.length > 0 && <div className="ph grain" data-img="1" data-label=""
            role="img" aria-label={images[imgIdx]?.label}
            style={{ aspectRatio: '4/5', '--img': `url(${images[imgIdx]?.src})` }} />}
        </div>
          ...
          {(p.options || []).map(option => (
            <div key={option.name} style={{ marginBottom: 24 }}>
              <div className="mono" style={{ fontSize: 10, color: 'var(--fg-3)', marginBottom: 12 }}>
                {option.name.toUpperCase()} · {String(selection[option.name.toLowerCase()] || '').toUpperCase()}
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {(option.values || []).map(value => {
                  ...
                  return <button key={value} onClick={() => chooseOption(option.name, value)}
                    ...
                    style={{ minHeight: 40, padding: '0 14px', border: `1px solid ${active ? 'var(--fg)' : 'var(--line-2)'}`, background: active ? 'var(--fg)' : 'transparent', color: active ? 'var(--bg)' : available ? 'var(--fg-2)' : 'var(--fg-4)', opacity: available ? 1 : 0.45, textDecoration: available ? 'none' : 'line-through', cursor: available ? 'pointer' : 'not-allowed' }}>
                    {value}
                  </button>;
                })}
              </div>
  ...
  return (
    <div className="page-fade">
      ...
      <div className="pdp-commerce-layout" style={{
        ...
        <div className="pdp-thumb-rail" style={{ display: 'flex', flexDirection: 'column', gap: 10, paddingTop: 0 }}>
          {images.map((img, i) => (
            <button key={i} onClick={() => setImgIdx(i)}
              ...
              className="ph tiny" data-img="1" data-label=""
              style={{
                width: 70, height: 70, aspectRatio: '1', cursor: 'pointer',
                '--img': `url(${img.src})`,
        ...
        {/* Main image */}
        <div className="pdp-main-media" style={{ paddingLeft: 0 }}>
          <div className="ph grain pdp-main-image" data-img="1" data-label=""
            style={{ aspectRatio: '1 / 1', height: 'auto', minHeight: 0, position: 'relative', '--img': `url(${images[imgIdx].src})` }}>
        ...
        {/* Details panel */}
        <div className="pdp-details-panel" style={{ paddingLeft: 0, position: 'sticky', top: 88, alignSelf: 'flex-start' }}>
