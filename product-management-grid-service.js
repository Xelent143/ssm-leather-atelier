...
function createProductManagementGridService(options = {}) {
  ...
  function projectEditor(product, drafts = []) {
    ...
    return {
      ...
      gender: product.classification?.gender?.value || product.organization?.gender || '',
      ageGroup: product.classification?.ageGroup?.value || product.organization?.ageGroup || '',
  ...
  function projectLegacy(product) {
    ...
    return {
      ...
      gender: product.merchantAttributes?.gender || product.gender || '',
      ageGroup: product.merchantAttributes?.age_group || product.ageGroup || '',
