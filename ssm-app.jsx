...
const SSM_INITIAL_PRODUCT = window.__SSM_INITIAL_PRODUCT__ || null;
const productFromSlug = (slug) =>
  (SSM_INITIAL_PRODUCT?.slug === slug ? SSM_INITIAL_PRODUCT : null)
  || SSM_PRODUCTS.find(p => p.slug === slug)
  || null;
...
function applySEO(view, params) {
  ...
  if (view === 'pdp') {
    ...
    title = p.seoTitle || SSM_SEO.pdp.title
      .replace('%name%', p.name)
      .replace('%cat%', p.cat)
      .replace('%gender%', p.gender);
    desc = (p.seoDescription || p.story?.piece || p.publicDescription || p.blurb || '').slice(0, 158);
  }
  if (view === 'article') {
    const article = articleFromParams(params);
    if (article) {
  ...
  const canonicalUrl = view === 'pdp' && params?.product?.canonicalUrl
    ? params.product.canonicalUrl
    : `${window.location.origin}${cleanPathForView(view, params)}`;
