    ...
    return `<div class="pe-page">
      ...
      <form id="pe-form" class="pe-layout">
        <main class="pe-main">
          ...
          <section class="card pe-card"><h2>Category</h2><div class="pe-grid-2">${workspace?.taxonomy?.length ? `<label>Category<input name="organization.category" required list="pe-taxonomy-options" value="${esc(product.organization.category)}" placeholder="Select synced category"><datalist id="pe-taxonomy-options">${workspace.taxonomy.map((item) => `<option value="${esc(item.name)}" label="${esc(item.hierarchyPath)}"></option>`).join('')}</datalist><small>Published MOTOGRIP taxonomy · searchable hierarchy path</small></label>` : input('Category', 'organization.category', product.organization.category, 'text', 'required')}${select('Gender', 'organization.gender', product.organization.gender, ['Men', 'Women', 'Unisex'])}</div></section>
