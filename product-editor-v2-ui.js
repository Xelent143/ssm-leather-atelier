    ...
    return `<div class="pe-page">
      ...
      <form id="pe-form" class="pe-layout">
        ...
        <aside class="pe-side">
          ...
          <section class="card pe-card"><h2>Quick listing readiness</h2><p class="muted">Only these five product fields are required. Product Identity, approval, Release and Knowledge Lock remain mandatory governance steps.</p><ul class="pe-checks"><li class="${product.title ? 'done' : ''}">Product name</li><li class="${product.organization.category ? 'done' : ''}">Category</li><li class="${Number(product.pricing.price) > 0 ? 'done' : ''}">Price</li><li class="${product.media.some((item) => item.featured) ? 'done' : ''}">Primary image</li><li class="${product.organization.status !== 'archived' ? 'done' : ''}">Product status</li></ul></section>
          <section class="card pe-card"><h2>Publishing requirements</h2><ul class="pe-checks"><li class="done">Product UUID reserved</li><li class="${identity?.state === 'locked' ? 'done' : ''}">Locked Product Identity</li><li>Trusted Product Release</li><li>Valid Knowledge Lock</li><li class="${product.title && product.media.some((item) => item.featured) && product.organization.category && Number(product.pricing.price) > 0 ? 'done' : ''}">Quick listing fields complete</li><li class="${product.workflowState === 'approved' ? 'done' : ''}">Owner-approved revision</li></ul><details class="pe-owner-guide"><summary>Create and publish your first product</summary><ol><li>Complete the five Quick Listing fields and save.</li><li>Submit for review and approve as Owner.</li><li>Open Product Detail to approve and lock Product Identity.</li><li>Create the Product Version, Approval Request, Release and Knowledge Lock.</li><li>Return here, preview, then Approve & Publish.</li></ol></details></section>
        </aside>
      </form>
