...
const navigationGroups = [
  ...
  ['Commerce', [
    ...
    ['products', '□', 'Product Management', '/admin/products', 'active'],
    ['product-editor', '+', 'Add Product', '/admin/product-editor/new', 'active'],
    ['catalog-review', '◎', 'Catalog Review', '/admin/catalog/review', 'active'],
    ['current-products', '↺', 'Legacy Product Manager', '/admin/products/current', 'existing'],
...
function renderMvpProducts() {
  if (!state.productGrid) {
    return `${PageHeader('Product Management', 'Professional product management for MOTOGRIP OS.', '', 'active')}${LoadingSkeleton()}`;
  }
  ...
  return `
    <div class="pg-page">
      ${PageHeader('Product Management', 'Manage products, variants, merchandising status, and website synchronization from one fast workspace.', '<a class="btn" data-route="catalog-review" href="/admin/catalog/review">Catalog Review</a><a class="btn" data-route="current-products" href="/admin/products/current">Legacy Product Manager</a><button class="btn primary" id="open-product-editor">Add Product</button>', 'active')}
...
function renderCategories() {
  ...
  const canEdit = state.taxonomy.permissions.edit;
  const canCreate = state.taxonomy.permissions.create;
  return `<div class="taxonomy-page">
    ${PageHeader('Categories & Collections', 'Manage website taxonomy, nested navigation, assignments, SEO, and synchronization.', `${canEdit?'<button class="btn" id="taxonomy-sync">Sync website</button>':''}${canCreate?'<button class="btn primary" id="add-category">Add category</button>':''}`, 'active')}
    ${state.taxonomy.reconciliation ? `<div class="taxonomy-reconciliation"><strong>Website source</strong><span>${state.taxonomy.reconciliation.sourceCount} unique names</span><span>${state.taxonomy.reconciliation.importedCount} imported</span><span>${state.taxonomy.reconciliation.deduplicatedCount} duplicates consolidated</span><span>${state.taxonomy.reconciliation.assignmentCount} assignments</span></div>` : ''}
    ...
    <div class="card pg-table-card"><div class="pg-table-wrap"><table class="pg-table taxonomy-table"><thead><tr><th><input id="category-select-page" type="checkbox"></th><th>Category</th><th>Parent / path</th><th>Products</th><th>Children</th><th>Status</th><th>Visibility</th><th>SEO</th><th>Sync</th><th>Updated</th><th>Actions</th></tr></thead><tbody>
      ${page.map((category) => `<tr data-category-row="${category.id}" draggable="true">
        ...
        <td><details class="pg-actions"><summary>•••</summary><div><button data-category-action="preview" data-category-id="${category.id}">Preview website</button><button data-category-action="view-products" data-category-id="${category.id}">View products</button>${canEdit?`<button data-category-action="edit" data-category-id="${category.id}">Edit</button><button data-category-action="move" data-category-id="${category.id}">Move</button><button data-category-action="reorder" data-category-id="${category.id}">Reorder</button><button data-category-action="hide" data-category-id="${category.id}">Hide</button><button data-category-action="archive" data-category-id="${category.id}">Archive</button><button data-category-action="restore" data-category-id="${category.id}">Restore</button>`:''}${canCreate?`<button data-category-action="add-child" data-category-id="${category.id}">Add subcategory</button><button data-category-action="duplicate" data-category-id="${category.id}">Duplicate as draft</button>`:''}<button data-category-action="activity" data-category-id="${category.id}">Activity</button><button data-category-action="sync-history" data-category-id="${category.id}">Sync history</button>${state.taxonomy.permissions.delete?`<button class="danger" data-category-action="delete" data-category-id="${category.id}">Safe delete</button>`:''}</div></details></td>
    ...
    ${state.categorySelection.size?`<div class="pg-bulk-bar"><strong>${state.categorySelection.size} selected</strong>${canEdit?'<button data-category-bulk="edit">Bulk edit</button><button data-category-bulk="move">Move to parent</button><button data-category-bulk="reorder">Reorder</button><button data-category-bulk="hide">Hide</button><button data-category-bulk="archive">Archive</button><button data-category-bulk="restore">Restore</button>':''}${state.taxonomy.permissions.delete?'<button class="danger" data-category-bulk="delete">Safe delete</button>':''}<button data-category-bulk="export">Export</button><button id="clear-category-selection">Clear</button></div>`:''}
...
function renderCategoryEditor() {
  ...
  return `<div class="pg-modal-backdrop"><section class="pg-modal category-editor" role="dialog" aria-modal="true"><div class="pg-drawer-head"><div><span class="eyebrow">${isNew?'New taxonomy record':'Category Editor'}</span><h2>${escapeHtml(model.name||'Untitled category')}</h2></div><button class="icon-button" data-close-category>×</button></div>
    <form id="category-editor-form"><div class="category-editor-grid"><main>
      ...
      <section class="card card-pad"><h3>Manual product assignments</h3><input id="category-product-search" placeholder="Search title, SKU, brand, type, handle, or source"><div class="category-products">${state.taxonomy.products.map((product)=>`<label data-category-product-label="${escapeHtml(`${product.title} ${product.sku} ${product.brand} ${product.productType} ${product.handle} ${product.sourceIdentity}`.toLowerCase())}"><input type="checkbox" name="productAssignment" value="${product.id}" ${assigned.has(product.id)?'checked':''}><img src="${escapeHtml(product.image||'/assets/generated/leather-detail.png')}" alt=""><span><strong>${escapeHtml(product.title)}</strong><small>${escapeHtml(product.sku||'Missing SKU')} · ${escapeHtml(product.status)} · ${escapeHtml(product.brand||'No brand')}</small><small>${escapeHtml(product.productType||'No type')} · Inventory ${Number(product.inventory||0)} · ${escapeHtml(product.sourceIdentity||'unknown')} · ${escapeHtml(product.syncState||'unknown')}</small></span></label>`).join('')}</div></section>
...
function bindCategoryManagement() {
  ...
  document.querySelectorAll('[data-category-action]').forEach((button)=>button.addEventListener('click',async()=>{
    ...
    try {
      if(['edit','move','view-products'].includes(action)){state.categoryEditorId=id;render();if(action==='move')requestAnimationFrame(()=>document.querySelector('[name=parentId]')?.focus());if(action==='view-products')requestAnimationFrame(()=>document.getElementById('category-product-search')?.scrollIntoView({behavior:'smooth'}));return;} if(action==='add-child'){state.categoryEditorId=false;render();requestAnimationFrame(()=>{document.querySelector('[name=parentId]').value=id;});return;}
      if(action==='preview'){window.open(`/collections/${category.slug}`,'_blank','noopener');return;} if(['activity','sync-history'].includes(action)){state.categoryActivity=await api(`/api/admin/categories/${id}/activity`);render();return;}
      if(action==='reorder'){const sortOrder=window.prompt('Enter the new sort order:',String(category.sortOrder));if(sortOrder===null)return;await api(`/api/admin/categories/${id}`,{method:'PUT',body:JSON.stringify({sortOrder:Number(sortOrder),expectedRevision:state.taxonomy.storeRevision})});state.taxonomy=await api('/api/admin/categories');toast('Category order updated');render();return;}
  ...
  document.querySelectorAll('[data-category-bulk]').forEach((button)=>button.addEventListener('click',async()=>{
    ...
    let values={};let effectiveAction=action;let confirmation;
    if(action==='edit'){const status=window.prompt('Set status: active, hidden, archived, or draft');if(!status)return;values={status};}
    if(action==='move'){const parentName=window.prompt('Enter an exact parent category name, or leave blank to move to root:','');if(parentName===null)return;const parent=state.taxonomy.categories.find((item)=>item.name.toLowerCase()===parentName.trim().toLowerCase());if(parentName.trim()&&!parent)return toast('Parent category was not found');values={parentId:parent?.id||null};effectiveAction='edit';}
    if(action==='reorder'){const starting=Number(window.prompt('Starting sort order:','1'));if(!Number.isFinite(starting))return;try{let offset=0;for(const categoryId of state.categorySelection){await api(`/api/admin/categories/${categoryId}`,{method:'PUT',body:JSON.stringify({sortOrder:starting+offset,expectedRevision:state.taxonomy.storeRevision})});state.taxonomy=await api('/api/admin/categories');offset+=1;}state.categorySelection.clear();toast('Category order updated');render();}catch(error){toast(error.message);}return;}
    if(action==='delete'){confirmation=window.prompt('Only safe empty categories can be deleted. Type DELETE to confirm.');if(confirmation!=='DELETE')return;}
    try{state.taxonomy=await api('/api/admin/categories/bulk',{method:'POST',body:JSON.stringify({categoryIds:[...state.categorySelection],action:effectiveAction,values,confirmation})});state.categorySelection.clear();toast('Bulk category action complete');render();}catch(error){toast(error.message);}
  }));
  ...
  activityStream.addEventListener('message', async (event) => {
    try {
      ...
      if (message.type === 'permissions.updated') {
        ...
        await refreshProductGrid();
        try { state.taxonomy = await api('/api/admin/categories'); } catch {}
        if (state.productEditorWorkspace?.product) {
          try {
            state.productEditorWorkspace = await api(`/api/admin/product-editor-v2/products/${state.productEditorWorkspace.product.id}`);
          } catch {}
        }
